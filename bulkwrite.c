...
static int alloc_other_requests(struct usb_composite_dev *cdev,
			  struct f_loopback *loop)
{
	struct usb_request *out2_req, *in2_req;
	int i;
	int result = 0;

	/*
	 * allocate a bunch of read/write buffers and queue them all at once.
	 * we buffer at most 'qlen' transfers; 	 
         */
	for (i = 0; i < loop->qlen && result == 0; i++) {
		...
		in2_req = lb_alloc_ep_req(loop->in2_ep, loop->buflen);
		if (!in2_req)
			goto fail;
		...
		out2_req->context = NULL;
		in2_req->context = NULL;
		...
		if (result) {
        	...
        	result = usb_ep_queue(loop->in2_ep, in2_req, GFP_ATOMIC);
		if (result) {
			ERROR(cdev, "%s queue req --> %d\n",
					loop->in2_ep->name, result);
			goto fail_in2;
		}
	}
	return 0;
...
fail_in2:
	free_ep_req(loop->in2_ep, in2_req);
fail_out2:
...
static int alloc_lb_requests(struct usb_composite_dev *cdev,
			  struct f_loopback *loop)
{
	struct usb_request *out1_req, *in1_req;
	int i;
	int result = 0;
	...
	for (i = 0; i < loop->qlen && result == 0; i++) {
		...
		in1_req->context = out1_req;
                ...
                in1_req->length = out1_req->length; 
		...
		if (result) {
        	...
        	result = usb_ep_queue(loop->in1_ep, in1_req, GFP_ATOMIC);
		if (result) {
			ERROR(cdev, "%s queue req --> %d\n",
					loop->in1_ep->name, result);
			goto fail_in1;
		}
	}
        in_queue_cnt += loop->qlen;
	return 0;
...
static int alloc_next_requests(struct usb_composite_dev *cdev,
                          struct f_loopback *loop, struct usb_ep *ep)
{
     ...
     for (i = 0; i < loop->qlen && result == 0; i++) {
		...
		req = lb_alloc_ep_req(ep, loop->buflen);
		if (!req)
			goto fail;
...
//				test_check();
			...
			ep = loop->in2_ep;
                        ...
                        if(loop->qlen <= 1) {
                            ...
                            opts = container_of(f->fi, struct f_lb_opts, func_inst);
                            loop->qlen = opts->qlen;
                            if(in_queue_cnt > loop->qlen)
                                alloc_next_requests(cdev, loop, ep);
                            else
                                alloc_lb_requests(cdev, loop);
                        } 
                        if(likely(prev_req != NULL)) {
                            usb_ep_free_request(ep, prev_req);
                        }
                        prev_req = req;
                        return;
        	}
                ...
                const struct usb_ctrlrequest *ctrl)
{
    ...
    case 0x82:
        ...
        /* ep queue for returning transfer descriptor when polled */ 
        bulkreq = lb_alloc_ep_req(loop->in1_ep, 4);
        if(bulkreq != NULL) {
            ...
            if(rv < 0) {
                ERROR(c->cdev, "bulk EP IN req failed \n");
            }
        }
    break;
    }
